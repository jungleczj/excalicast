...
### 目标用户画像（2026-05-20 复盘确认）

**产品定位**：「**白板 + 录屏的混合工具**」—— 不放弃 Excalidraw 集成（护城河），同时新增屏幕录制能力。

**核心用户画像**：用 Excalidraw 讲技术 / 设计 / 教育的创作者。他们的工作流已经在白板里，需要把讲解过程（白板讲解 + 代码 demo + 其他工具演示）录下来发给学生 / 用户 / 同事。

| 用户类型 | 典型用例 | 使用强度 |
|---|---|---|
| 技术教程作者 | 录 VS Code + Excalidraw 架构图，发 B 站 / YouTube | 主要 |
| 知识区博主 / 课程作者 | 录长课程，混合白板讲解 + 屏幕演示 | 主要 |
| 设计师 | 录 Figma + Excalidraw 流程图 demo | 中等 |
| 产品 demo / pitch | 录短视频解释产品（含白板配图） | 中等 |

**护城河 vs. 竞品**：
- Loom（1400 万用户）没有白板集成
- Excalidraw 不能录屏
- Excalicast = "在白板里启动屏录"，一个 tab 走完工作流

**功能范围决策**（2026-05-20 复盘后用户确认）：维持本 spec 完整功能集，不接受砍减。即使有"炫但低效用"的风险（如 AI 大纲），仍保留为 Pro 卖点。
...
- ❌ **RegionSelector 的裁剪框必须严格绑定所选比例**（9:16 → 框只能拖动 9:16 锁定的矩形；不允许"自由拖任意比例"）。「原始」模式跳过 RegionSelector
