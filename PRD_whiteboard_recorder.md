...
- **2026-08-23｜Mac 原生多轨录制阶段检查点（在建）**：独立工作树 `codex/macos-ai-teaching-studio` 已接通 Electron → Swift Helper 的版本化权限、来源、设备、预检、开始、停止、压力状态与项目恢复通道；屏幕/窗口走 ScreenCaptureKit，屏幕和摄像头分别走强制硬件 VideoToolbox H.264，麦克风与系统音频走独立 AVFoundation/AAC 轨，所有轨共用单调时钟并按约 2 秒短分段原子写清单。实时队列只保留最新 2 帧，编码/mux/写盘不进入 React、Canvas 或 Electron IPC。补齐静态窗口 `.idle` 流的“一次截图种子 + 低频关键帧心跳”，避免静态教学画面产生 0 帧；SCScreenshotManager 设 2 秒看门狗并回退一次性 CoreGraphics，截图全黑且 SCStream 仍无完整帧时明确返回 `captureSourceUnavailableOrProtected`，禁止生成黑色视频假成功。并发媒体回调统一串行 mux，跨轨 manifest 更新加锁，受保护源失败后生命周期回到 idle。摄像头/麦克风用稳定设备 ID 枚举；摄像头在启动前检查权限、硬件编码与原生格式，锁定不低于请求分辨率且覆盖请求帧率的 `activeFormat`，不能静默选低清格式；Electron 请求解析已有专项测试保证系统音频、麦克风、摄像头及其设备/格式参数完整传给 Helper。本机已验证内置摄像头支持 1280×720 15–30fps（因此 720p24 格式可用），但权限仍为 `not-determined`。桌面录制资源门已升级为录制优先的双向租约：云上传/下载、字幕 ASR、讲义、英文配音、要点生成、光标分析与本地导出在空闲时持有可取消租约；开始原生录制前先中止并等待这些任务释放，导出等协调任务进入可恢复 `paused`，拒绝释放的任务不会与录制并跑。带租约的 Supabase 上传无论大小都走真正支持 abort 的 TUS，摄像头代理转码逐帧检查取消；`recording/stopping` 期间新任务继续等待，浏览器无桌面桥时保持原行为。恢复会过滤丢失/空分片、清除 `.staging` 半分片、把 `recording/finalizing` 原子回写为 `interrupted`；manifest 轨道改为稳定字符串键对象，并兼容读取首版 Swift 键值交替数组。分片提交升级为可重放事务：先原子写 `.commit-journal`，再提升媒体文件、检查点 manifest，最后删除日志；进程在移动前或 manifest 写入前崩溃时，恢复会安全提升或补登记媒体，并拒绝任何越出对应轨道目录的日志路径。已用此前真实录制的旧 manifest 验证 12 个 MP4 全部保留且改写为新格式。运行中每 2 秒监测磁盘：>5GB 正常、1–5GB 警告、≤1GB 临界；压力状态同时报告可用空间、待写/累计字节及最近/最高分片提交延迟。临界时生命周期先进入 `stopping`，冲刷各轨后把项目标为 `interrupted` 并回到 `idle`，保留已完成分片和 `critical_disk_space` 原因。manifest 还持久化屏幕/摄像头实际输出配置、双音频开关、硬件确认、初始磁盘和最终压力；已真实验证受锁屏保护源启动失败后，恢复项目仍完整记录 1280×720@30 H.264、硬件确认和 `interrupted`，不会丢失诊断证据。新增媒体轨就绪双闸门：摄像头必须有首帧成功进入硬件编码器、麦克风必须有首样本进入 AAC writer 后，Helper 才返回 `recording`；停止时已勾选的摄像头/麦克风必须存在可恢复分片，否则项目保持 `interrupted`。失败通过 IPC 返回稳定 `errorCode + errorTrack`，本机已真实验证未授权摄像头返回 `camera_permission_required / camera` 且生命周期仍为 idle。系统音频允许合法静音，不以零样本误报故障。新增 `project.validate.v1` 自动验收每条分片的存在性、字节数、H.264/HEVC/AAC 类型、AVFoundation 首样本真实解码和轨内连续性；对旧的 12 分片真机项目验证出 12/12 H.264 首帧可解码，同时定位到静态心跳曾把 500ms 定时器抖动写成最高 534122µs 时间轴缺口。心跳已改为固定两秒媒体边界（桥接帧 + 边界关键帧），停止时再延伸到真实停止时间，定时器晚醒不再污染媒体时间戳；待会话解锁后用新录制复验缺口归零。新增可配置原生浸泡测试入口，直接采样 Helper RSS、编码待处理帧、写盘积压和分片提交延迟，停止后强制执行真实媒体解码与连续性验收；短测与 60 分钟测试共用同一判定器，线性内存增长、持续编码积压、64MB 写队列或 2 秒写延迟都会令命令非零退出。Swift 合约、硬件探测、并发恢复、TypeScript、Desktop build 与 Desktop/任务中心 Playwright 契约已通过；当前 macOS 26 测试会话存在 `Display 1 Shield`（系统窗口截图同样失败），正向 ScreenCaptureKit 真机录制、摄像头授权后真实分片与 60 分钟浸泡仍属于阶段 3 未完成门槛。
- **2026-08-23｜Mac 透明 Excalidraw 白板阶段检查点（在建）**：Electron 新增透明、无边框、跨空间置顶、内容保护的全显示器 Ink/Full Board 窗口，支持绘制/点击穿透切换；桌面工作区仅在原生桥存在时显示入口，Web 行为不变。透明白板直接复用现有 `Whiteboard` 与 Excalidraw 原生完整工具面，内置 Library/sidebar 不再被 Web 专用 CSS 误隐藏；白板背景与笔迹透明度独立，主窗口和浮层可实时双向控制。Electron 自动把可见私有浮层原生窗口 ID 合并到捕获请求，Swift 只匹配仍可共享的窗口并交给 `SCContentFilter(display:excludingWindows:)`，不依赖后期遮挡。录制时仅收集版本发生变化的 Excalidraw 元素、文件、视口和指针/激光事件，512 条有界队列按 100ms 批量提交；批次使用录制相对微秒时间写入原生 `excalidraw-events` 分段，沿用原子 manifest、提交日志与崩溃恢复。失败批次恢复到队首重试；停止前由主进程发 flush 请求并等待白板确认及原生提交队列排空。当前 27 项桌面契约、2 项真实页面渲染/工作区入口、TypeScript、Desktop build、Swift 合约与完整 helper build 已通过；下一检查点为事件轨场景往返和 60fps 确定性重放/导出。
