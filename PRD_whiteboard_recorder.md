...
- **2026-08-23｜Mac 原生多轨录制阶段检查点（在建）**：独立工作树 `codex/macos-ai-teaching-studio` 已接通 Electron → Swift Helper 的版本化权限、来源、设备、预检、开始、停止与压力状态通道；屏幕/窗口走 ScreenCaptureKit，屏幕和摄像头分别走强制硬件 VideoToolbox H.264，麦克风与系统音频走独立 AVFoundation/AAC 轨，所有轨共用单调时钟并按约 2 秒短分段原子写清单。实时队列只保留最新 2 帧，编码/mux/写盘不进入 React、Canvas 或 Electron IPC。补齐静态窗口 `.idle` 流的“一次截图种子 + 低频关键帧心跳”，避免静态教学画面产生 0 帧；SCScreenshotManager 设 2 秒看门狗并回退一次性 CoreGraphics，截图全黑且 SCStream 仍无完整帧时明确返回 `captureSourceUnavailableOrProtected`，禁止生成黑色视频假成功。并发媒体回调统一串行 mux，跨轨 manifest 更新加锁，受保护源失败后生命周期回到 idle。摄像头/麦克风用稳定设备 ID 枚举；摄像头在启动前检查权限、硬件编码与原生格式，锁定不低于请求分辨率且覆盖请求帧率的 `activeFormat`，不能静默选低清格式；Electron 请求解析已有专项测试保证系统音频、麦克风、摄像头及其设备/格式参数完整传给 Helper。本机已验证内置摄像头支持 1280×720 15–30fps（因此 720p24 格式可用），但权限仍为 `not-determined`。新增桌面录制资源门：云上传、字幕 ASR、讲义、英文配音、光标分析与本地导出在每次启动或轮询前读取原生捕获状态，`recording/stopping` 期间只等待且可取消，不发起重网络/CPU/磁盘工作；浏览器无桌面桥时保持原行为。Swift 合约、硬件探测、并发恢复、TypeScript、Desktop build 与 Desktop/任务中心 Playwright 契约已通过；当前 macOS 26 测试会话存在 `Display 1 Shield`（系统窗口截图同样失败），正向 ScreenCaptureKit 真机录制、摄像头授权后真实分片与 60 分钟浸泡仍属于阶段 3 未完成门槛。
