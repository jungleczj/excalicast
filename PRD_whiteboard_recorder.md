...
**作者**：—  
**最后更新**：2026-06-21  
**变更**：v0.8.7 - 文档整理：本轮要点沉淀进 **CLAUDE.md（+ AGENTS.md 双写）** 与 PRD 并分流——CLAUDE 补提词器/vosk 架构、UI 设计系统约定、ISR/会员刷新/付款测试 Q、禁止项；PRD 新增 **F5 提词器** 章节并重写 F1/F2/F3 与导出页 UI 正文为最新行为。详见「## 十一」最新一条。  
**历史变更**：v0.8.6 - UI 统一(P0+P1)：字幕面板/档位徽标调色并入手绘纸-墨令牌（去蓝色渐变与 Tailwind 语义色）；抽共享 `Modal` 壳（`--overlay` 令牌）统一 5 个弹窗的遮罩/阴影/圆角/关闭钮。  
...
**录制条工具**（录制前 / 录制中均可切）：摄像头、麦克风软静音、激光笔、**演示缩放**、**提词器**。
- **演示缩放**：开「演示缩放」开关 或 `Alt/⌘ + 双击` 画布某处 → 放大约 2× 聚焦该点，再次双击还原（讲解局部时用）。
- **提词器**：叠加智能跟读提词器（见 F5）。
...
- 前端方案：浏览器端合成（`ffmpeg.wasm` + WebCodecs，无需上传）
- 流程：按事件流逐帧渲染 Canvas → 编码 → 混入音频轨道
- 预计耗时：10 分钟录制约需 2-5 分钟本地渲染
- 进度条分阶段实时反馈，支持后台处理

#### 导出页 = 编辑器（录制后）

导出页是一个轻量编辑器（详见「## 十、#4」），导出前可调：

- **时间轴主流剪辑**：播放头 scrub（标尺 / 片段轨 / 小三角手柄都可拖）、**Split 切一刀**、删片段、片段**边缘 trim**；预览随拖动实时刮擦、跳过被删段。
- **分辨率 / 格式 / 画质面板**：标清→4K 全档位 + MP4 / WebM / GIF + 帧率 / 码率；录制端一律高采集（摄像头 1280² / 工作区外壳 pixelRatio≥2）保证各分辨率都清晰。
- **多选比例导出**：录一次，勾选多个比例（与录制 Setup 的全 10 预设一致），依次生成并下载；摄像头按「较短边」相对裁切框缩放，预览=录制一致、竖屏不过小。

#### 边界条件

- 导出 MP4 分辨率默认 1080p，按画幅比例决定宽高（见 F1.2）；可在格式面板改清晰度
- 无音频录制时：导出静音视频
...
```
音频 → 阿里千问 / DashScope（paraformer-v2）→ SRT 字幕 → 与操作序列时间戳对齐 → 嵌入导出
```

> 字幕统一用千问，**不用 Whisper**（产品级硬约束）。入口为导出页「字幕」Tab，**内联展示**（不再弹窗）。
...
- 阿里千问 paraformer-v2，中文识别效果好、支持时间戳 / SRT
- 语言提示中 / 英；生成后可下载 SRT / 重新生成 / 从录制移除
- 字幕清洗为单行，避免双重换行

#### 边界条件

- 无音频时：字幕功能不可用，灰态展示
- 音频经服务端代理调用 DashScope（API Key 不出前端）
...
录制时叠加在画面上的提词器（对标开源 macOS app **Textream**）。核心是**智能跟读**：你照着讲稿念，**当前词实时高亮、并平滑滚到中央**——说到哪亮到哪、**停说就停滚**、按语速自适应、容忍跳读 / 误读 / 小幅回读。是**前端录制辅助，不是字幕来源**，不占会员门控（**所有用户免费**）。

#### 关键交互

- **一个麦克风**：录制麦克风一旦在线（已授权、未静音）即**自动进入跟读**，同一路音频既录进视频又驱动高亮，不需要第二个麦克风开关。也可在不录制时手动点「✨ 智能跟读」启动（排练用）。
- **刘海条停靠 / 全屏无缝**：可收成顶部居中、从视口顶沿垂下的黑色单行窄条（Dynamic-Island 观感）；Mac 默认停靠。全屏演示时窄条与 macOS 刘海黑条无缝相连、"挂"在刘海下方。刘海条可拖动 / 拉长。
- **语言**：按讲稿内容自动判定中 / 英，亦可手动切换（应对中英夹杂）。
- **弹出浮窗**：Document Picture-in-Picture，可拖到屏幕任意处置顶。
- 控件：智能跟读、常速滚动（非录制兜底）、字号、透明度、编辑讲稿、刘海 / 全屏 / PiP。

#### 技术要点（详见 CLAUDE.md「7. 提词器」）

- 识别引擎**编排**：**Google（webkitSpeechRecognition）为首**——新 Chrome 用 `start(audioTrack)` 吃录制的 `MediaStreamTrack`、云端识别不占本地 CPU；**看门狗**在网络错（大陆不可达）/被拒/无结果时**自动回落离线 vosk**（WASM，本地、不传云、任意地区可用，AudioWorklet 取帧不卡）。
- 对齐为**纯函数**（中文按字 / 西文按词、指针只进不退、容跳读误读），可离线测（node 9/9）；真机用合成中文音频当假麦克风做 E2E。

#### 产品边界（用户须知）

- 智能跟读用浏览器 / vosk，**与后录的千问字幕管线无关**，不违反「字幕统一用千问」约束。
- 网页**无法占用物理刘海**、Electron 不支持 Web Speech；**真·窗口态常驻刘海 + 端侧识别需原生 app**，网页极限是「全屏无缝刘海条」。
...
> 注：原列「视频编辑（剪辑）MVP 不做」已落地——导出页时间轴支持主流剪辑（Split / 删段 / 边缘 trim / 播放头与片段轨可拖 scrub），见 F2。支付也已接入 Creem（见「## 十一」）。
...
**纸-墨极简手绘**：暖白纸底 + 炭墨线条、硬阴影（非模糊）、略不方正圆角、令牌驱动。**所有颜色 / 圆角 / 阴影 / 字体走 `globals.css :root` 的 CSS 变量**，统一复用 `.btn-sketch` / `MonoTag` / 共享 `Modal` / `TierBadge`（详规则与禁止项见 CLAUDE.md「8. UI 设计系统约定」）。

- **克制的浮层**：录制控制栏、人像窗口、字幕 / 提词器层叠加在白板上，不遮挡创作区域
...
- **顶栏**：返回 / Logo / 项目名内联可编辑（复用 `updateRecordingTitle`）/ **分享按钮**（`ShareButton`，Max；非 Max 点击开 Max 升级）/ **统一会员标 `TierBadge`**（FREE/PRO/MAX，与 AppHeader 一致）。（旧版的假「分享/导出」切 Tab 按钮已删——导出动作在右列面板。）
- **左列**：`ExportPreview`「成片」播放器（进度条可拖 scrub）+ `Timeline`（任意多段裁剪，见下）+ 删除录制。
- **右列分级 Tab**（按 `useSubscription().tier`）：Export（工作区开关 + 比例**多选** + **格式与清晰度** + 含/无水印 + **单一真导出按钮** + 解锁提示）所有档可见；Captions=Pro（**内联**字幕面板）、Outline/Handout=Max，锁定档渲染升级块点开 `ProUpgradeModal`。**已去掉右侧底部的 PRO/MAX 高级捆绑卡片**——字幕/讲义/大纲功能各归其 Tab；升级入口收敛为「解锁并下载 + Tab 锁定块 + 升级弹窗套餐卡」。
...
- 分享链接的**创建入口在导出页顶栏的分享按钮**（`ShareButton`，Max）；只上传事件流 + 音频、不传 MP4
...
| AI 能力 | 无（提词器辅助录制） | 字幕 + AI 讲义（核心差异）+ 智能跟读提词器（免费辅助） |
| 分享 | 导出视频 | 操作流回放链接 + 讲义 |
...
- **2026-06-21｜文档整理：CLAUDE/AGENTS 约定补全 + PRD 提词器立章 + 正文对齐**：把本轮大量产出沉淀进文档并按职责分流。**CLAUDE.md（+ AGENTS.md 双写同步，二者仅标题/示例 model id 两行有意不同）** 补：模块小节「7. 提词器实时跟读」（4 文件分层 + Google 为首/vosk 回落编排 + 模型运维 + 网页硬边界）、「8. UI 设计系统约定」（纸-墨令牌 + 复用 Modal/TierBadge + P2/P3 技术债）、4 条常见问题 Q（ISR 禁 force-dynamic / dev 慢非 prod / 登录刷新档位 / 付款测试环境）、6 条禁止项（跟读必喂 audioTrack、Electron 无 Web Speech、跟读≠字幕、vosk 不入库、checkout 弹窗拦截、禁硬编码色）、目录与 vosk env 补全。**PRD** 新增功能章节 **F5 提词器（智能跟读，免费录制辅助）**，并按最新行为重写 F1（演示缩放 + 提词器）/ F2（时间轴剪辑 + 格式清晰度 + 多选导出）/ F3（千问 paraformer 取代旧 Whisper 表述 + 字幕内联）/ 十·#4 导出页 IA（分享上顶栏 + TierBadge + 去高级卡片）/ #5 分享入口 / 设计风格定位（纸-墨令牌）。涉及 `CLAUDE.md`、`AGENTS.md`、`PRD_whiteboard_recorder.md`（纯文档）。
