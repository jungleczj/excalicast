...
**开始任何功能开发、Bug 修复、重构、配置或文档修改前，都必须先完成一次本地版本检查点。**

- 先运行 `git status` 并确认当前分支和工作树。
- 工作区存在改动时，先将当前版本完整创建为本地提交，再开始新的修改。
- 工作区干净时，记录当前 `HEAD` 作为本次修改的基准，不创建空提交。
- 本地提交失败、存在未确认改动或工作区状态不明确时，不得开始后续文件修改。
- 除非用户明确要求，否则检查点只提交到本地，不自动推送远程。
...
**Excalicast 落地页必须严格对标 `craft.do` 当前官网首页的风格与排版。**

这是一条项目级长期约束，不是一次性视觉建议。后续任何落地页调整都必须遵守：
- **对标范围**：布局骨架、首屏节奏、浮动胶囊导航、居中衬线大标题、单一主 CTA、满屏大圆角纸感 Hero、五能力入口、人物场景跑马灯、大叙事卡片、价格区、FAQ 与页脚节奏。
- **允许原创**：文案语义必须是 Excalicast 原创；Logo、产品图、人物图和业务截图必须使用 Excalicast 自有资产，不复制 Craft 文案、Logo、人物、图片。
- **不可跑偏**：不要回到“功能说明书式”的密集模块、花哨渐变、过多色块、硬描边手绘风、AI 模板感卡片墙。
- **Hero 边界**：Hero 产品图使用用户指定的产品图资产；人物资产包只用于后续人物跑马灯/使用场景区，不用于 Hero。
- **工程边界**：落地页调整只改 UI、文案和静态展示资产；不得修改 API、数据库、Hooks、支付、录制、导出、云同步等业务逻辑。
